...
func TestContainerCapabilitySetPersistsAddAndDefaultDrop(t *testing.T) {
	...
	if !result.OK() || !result.Changed || !lab.ContainerCapabilityEnabled(service.Lab.Containers[0], "NET_ADMIN") {
		t.Fatalf("enable NET_ADMIN = %#v, container=%#v", result, service.Lab.Containers[0])
	}
	capabilities := service.Lab.Containers[0].Capabilities
	...
	if !result.OK() || !result.Changed || lab.ContainerCapabilityEnabled(service.Lab.Containers[0], "NET_RAW") {
		t.Fatalf("disable NET_RAW = %#v, container=%#v", result, service.Lab.Containers[0])
	}
	capabilities = service.Lab.Containers[0].Capabilities
	...
	if !result.OK() || !result.Changed || !lab.ContainerCapabilityEnabled(service.Lab.Containers[0], "NET_RAW") {
		t.Fatalf("re-enable NET_RAW = %#v, container=%#v", result, service.Lab.Containers[0])
	}
	if got := service.Lab.Containers[0].Capabilities; got == nil || len(got.Drop) != 0 || len(got.Add) != 1 {
